...
if (strcmp(model.modelo, 'LibSVM'))
    ...
    if strcmp(model.kernel,'Arccosine')
        %yf = svm_predict31(model, x);
        yf = svm_predict_cos2(model, model.xt, x);
    
    else
        [m, ~]=size(x);
        [mg, ~] = size(model(1).gama'); %transforma o vetor gama em um vetor coluna, questão de facilidade
        y1 = zeros (m , mg);
        for i = 1:m
            for j = 1:mg
                y1(i, j) = svm_validade2(model(1), x(i,:),j);  %Função svm_validade2 possui erro na função polinomial              
            end
        end
        yf=y1;
    end
