function escreve_saida_mt_ls( model, model2, param, param2, x1, y1, s, path, amostra, min_x1, max_x1, min_x2, max_x2, kernel)
...
fprintf(fileID, 'C      Gama        Epsilon         #SV         RHO\n');
%As linhas representam as saídas e as colunas C, gama, epsilon, #SV e RHO, 
%respectivamente
for i = 1:l
    fprintf(fileID, ' %10.6f     %10.6f      %10.6f      %i        %10.6f\n', param(i,1), param(i,2), param(i,3), length(model(i).alpha), model(i).b);
end
...
for i=1:l
    fprintf(fileID, '\nY %i\n', i);
    aux_model = [model(i).selector'; model(i).alpha'];
...
%respectivamente
for i = 1:l
    fprintf(fileID, ' %10.6f     %10.6f      %10.6f      %i      %10.6f\n', param2(i,1), param2(i,2), param2(i,3), length(model2(i).alpha), model2(i).b);
end
fprintf(fileID, 'Dados Normalização\nMin        Max\n');
[~, c_s]=size(s(1).meta_x);
for i = 1:c_s
...
for i=1:l
    fprintf(fileID, '\nY %i\n', i);
    aux_model = [model2(i).selector'; model2(i).alpha'];
    fprintf(fileID, '%i       %10.6f\n', aux_model);
end
