...
for i=1:amostra
    ...
    fprintf(fileID,'\nSample %i\n',i);
    if (MODEDat.kernel == 1)||(MODEDat.kernel == 6) % Kernel RBF, Cauchy
    ...
    elseif (MODEDat.kernel == 3) % Kernel Arc cosseno
        if tipo_sv == 1  % Classificadores
            result_amostra = [1:size(r(i).PSet', 2); r(i).PSet(:,1:1)'; r(i).PFront'];
    ...
    elseif (MODEDat.kernel == 5) % Kernel Hermitiano
        if (tipo_sv == 1) % Classificador                
            result_amostra = [1:size(r(i).PSet', 2); r(i).PSet(:,1:2)'; r(i).PFront'];
            fprintf(fileID,'%5s %15s %15s %20s %10s\n', 'Index', 'C', 'Order',  'Error', 'SV');
    ...
    elseif (MODEDat.kernel == 7)
        if (tipo_sv == 1) % Classificador
            result_amostra = [1:size(r(i).PSet', 2); r(i).PSet(:,1:3)'; r(i).PFront']; %Excluindo o epsilon do classificador

            fprintf(fileID,'%5s %10s %15s %15s %15s\n', 'Index','C','Gamma', 'Coef', 'Error', 'SV');
            fprintf(fileID,'%3i %15.8f %15.8f %15.8f %15.8f %15.8f\n',result_amostra);
            escreve_saida_classify(r(i).PSet, r(i).PFront, X1, Y1, nome_arq.path, i, min_x1, max_x1, MODEDat.kernel);
        ...
        elseif (tipo_sv == 2) % Regressor
            ...
            fprintf(fileID,'%5s %10s %15s %15s %20s %15s %15s \n','Index', 'C','Gamma', 'Coef', 'Epsilon', 'MSE', 'SV');
            fprintf(fileID,'%3i %15.8f %15.8f %15.8f  %15.8f %15.8f %15.8f \n',result_amostra);
