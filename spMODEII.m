...
% Design Procedure. PhD. Thesis (2014), Universitat Polit�cnica de 
% Val�ncia. Url: http://hdl.handle.net/10251/38248. Supervisors: Javier 
...
ScalingFactor = spMODEDat.Esc;          % Scaling Factor
CRrate        = spMODEDat.Pm;       % Crossover rate
...
JxParent = mop(Parent,spMODEDat); %Calling CostFunction.m
...
%         break;
%     end
    %% Plotando os gráficos de convergência
    aux1 = size(PFront);
    [l_x1,~] = size(spMODEDat.X1);

    if (n == 1)
        mse_worst = max(PFront(:,1));
        mse_worst = 1.5*mse_worst(1,1);
        corr_worst = 1.5*max(PFront(:,3));
        %corr_worst = max(arc_f(:,3));
        %corr_worst = 1.5*corr_worst(1,1);
    end
    hyper(n) = Hypervolume_MEX(PFront, [mse_worst, l_x1, 1]);
    ...
    sols(n) = aux1(1,1);
    porcentagem = (n+(spMODEDat.amostra_atual-1)*Generations)/(spMODEDat.amostra*Generations);
    waitbar(porcentagem,spMODEDat.waitbar,sprintf('%i%% along...',100*porcentagem));

    if (n>spMODEDat.MAXGEN)
        disp('Finish.')
        break;
    end
